crosswordApp.controller("detailUserAttendedController", ["$scope", "$routeParams", "rewards", "$location", "$validator",
    function($scope, $routeParams, rewards, $location, $validator){
        ...
        rewards.getUserDetail({
            ...
            $scope.user = data[0];
            $scope.user.alreadyWonReward = $scope.user.winOrLose;
                ...
                .success(function(){
                    ...
                    $scope.user.$updateUserWinOrLose({
                        ...
                        var messageObj = {
                            ...
                            apnMessage : data.apnMessage,
                            gcmMessage : data.gcmMessage,
                            mailMessage : data.mailMessage
                        };

                        $scope.back(messageObj);
                    },function(err){
                        //console.log(err);
                        $scope.errorMessage = err.data.message;
                    });
        ...
        // Back button event
        $scope.back = function(messageObj){
            var searchObj = angular.extend($location.search(), messageObj);
            $location.path("/rewards/edit/" + rewardId).search(searchObj);
        };
