//remoove all space include japanese space
function ReplaceSpace(str){
    this.t = str.replace(/　/g,'').replace(/\s{2,}/g,'').trim().replace(/[*'[\]\^%?#]/g,'').replace(/ /g,"");
    return this.t;
}
...
function WordElement(word, index){
    this.word = ReplaceSpace(word); // the actual word
    this.index = index; // use to map this node to its word or clue
}
...
function Crossword(words_in, clues_in){
    ...
    this.getSquareGrid = function(max_tries){
        ...
        var gameObject = {};
        gameObject.questions = clues_in;
        gameObject.answers = words_in;
        gameObject.grid = best_grid;
        return gameObject;
    }
    ...
    var minimizeGrid = function(){
        ...
        if(rows>cols){cols = rows;}else{rows = cols;}
        ...
        for(var r = r_min, r2 = 0; r2 < rows; r++, r2++){
            for(var c = c_min, c2 = 0; c2 < cols; c++, c2++){
                ...
                if(new_grid[r2][c2]){
                    new_grid[r2][c2]['row'] = r2;
                    new_grid[r2][c2]['col'] = c2;
                }

//                if(new_grid[r2][c2]){
//                    var index=0;
//                    if(new_grid[r2][c2]["across"]){
//                        index = new_grid[r2][c2]["across"]["index"];
//                        new_grid[r2][c2]["across"]["status"] = "fail";
//                        new_grid[r2][c2]["across"]["userAnswer"] = "";
//                    }
//                    if(new_grid[r2][c2]["down"]){
//                        index = new_grid[r2][c2]["down"]["index"];
//                        new_grid[r2][c2]["down"]["status"] = "fail";
//                        new_grid[r2][c2]["down"]["userAnswer"] = "";
//                    }
//                    new_grid[r2][c2]["usedCoin"] = false;
//                    new_grid[r2][c2]["question"] = clues_in[index];
//                    new_grid[r2][c2]["answer"] = words_in[index];
//
//                }
    ...
    // seems to work well
    //sort word theo length giam dan
    word_elements.sort(function(a, b){

        var l = b.word.length - a.word.length;
        return l;
    });
}
        ...
        for(var r = 0; r < grid.length; r++){
            ...
            for(var c = 0; c < grid[r].length; c++){
                ...
                if(cell == null){
                    ...
                    var css_class = "hasChar";
                    var is_start_of_word = (cell['across'] && cell['across']['is_start_of_word']) || (cell['down'] && cell['down']['is_start_of_word']);
                }

                if(is_start_of_word) {
                    css_class = css_class += " start-of-word";

                    html.push("<td class='" + css_class + "' char='" + char + "' location='" + r + ", " + c + "'>");
                    label++;			
                } else {
                    html.push("<td class='" + css_class + "' char='" + char + "' location='" + r + ", " + c + "'>");
                }
